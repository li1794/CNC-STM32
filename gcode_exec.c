		...
		if (stepm_getRemainLines() > 1)
		{
			...
			for (i = 0; i < STEPS_MOTORS; i++)
			{
				...
				if (i == 2)
				{
					...
					scr_gotoxy(1 + 2 * 10, 6); scr_printf("errZ:%f  ", encValmm - n);
...
static __INLINE int8_t findInAccelerationCrd(MVECTOR *p, MVECTOR *p_in, int32_t *smothdF, int32_t *frqStart)
{
	...
	for (i = 0; i < 3; i++)
	{
		if (p->frq[i] == 0 || df[i] < _smParam.smoothStartF_from0[i])
		{
			dt[i] = 0;
			continue;
		} // 写胁懈卸械薪懈褟 锌芯 泻芯芯褉写懈薪邪褌械 - 薪械褌. 蟹薪邪褔懈褌 褌芯褉屑芯卸械薪懈械 褍褋泻芯褉械薪懈械 芯褌褋褍褌胁褍械褌.
		step_df[i] = _smParam.smoothAF[i];
		dt[i] = df[i] / step_df[i]; // 胁褉械屑褟 薪械芯斜褏芯写懈屑芯械 薪邪 懈蟹屑械薪械薪懈械 褋泻芯褉芯褋褌懈
		for (j = 0; j < 3; j++)
		{ // 泻芯褉褉械泻褑懈褟 褍褋泻芯褉械薪懈褟 褋 褍褔械褌芯屑 芯褋褌邪谢褜薪褘褏 芯褋械泄.
			int32_t f;
	...
	for (i = 0; i < 3; i++)
	{
		if (max_time < dt[i])
		{
			crd = (int8_t)i;
			max_time = dt[i];
		} // 懈褖械屑 芯褋褜 褋 屑邪泻褋懈屑邪谢褜薪褘屑 胁褉械屑械薪械屑 褍褋泻芯褉械薪懈褟      
	}
	// 械褋谢懈 褉邪蟹薪懈褑邪 胁 褋泻芯褉芯褋褌褟褏 屑械薪褜褕械 褌芯泄, 锌褉懈 泻芯褌芯褉芯泄 屑芯卸薪芯 褋褌邪褉褌芯胁邪褌褜 褋 0 懈 薪械 斜褘谢芯 泻芯褉褉械泻褑懈泄
...
static __INLINE int8_t findOutAccelerationCrd(MVECTOR *p, MVECTOR *p_out, int32_t *smothdF, int32_t *frqStop)
{
	...
	{
		// 写胁懈卸械薪懈褟 锌芯 泻芯芯褉写懈薪邪褌械 - 薪械褌. 蟹薪邪褔懈褌 褌芯褉屑芯卸械薪懈械 褍褋泻芯褉械薪懈械 芯褌褋褍褌胁褍械褌
		// 懈 褌邪泻 卸械 蟹邪锌褉械褕械薪芯 褍褋泻芯褉械薪懈械 胁褘褕械 褋泻芯褉芯褋褌懈, 褍泻邪蟹邪薪薪芯泄 写谢褟 芯褌褉械蟹泻邪
		if (p->frq[i] == 0 || frq_out[i] >(int32_t)p->frq[i]) { dt[i] = 0; continue; }
		step_df[i] = _smParam.smoothAF[i];
		dt[i] = df[i] / step_df[i]; // 胁褉械屑褟 (msec) 薪械芯斜褏芯写懈屑芯械 薪邪 懈蟹屑械薪械薪懈械 褋泻芯褉芯褋褌懈
		for (j = 0; j < 3; j++) { // 泻芯褉褉械泻褑懈褟 褍褋泻芯褉械薪懈褟 褋 褍褔械褌芯屑 芯褋褌邪谢褜薪褘褏 芯褋械泄.
			int32_t f;
			if (i == j || p->steps[j] == 0) continue;
			f = (int32_t)((int64_t)step_df[i] * (int64_t)p->steps[j] / p->steps[i]);
			if (f >(_smParam.smoothAF[j] * 3 / 2)) {
	...
	for (i = 0; i < 3; i++) {
		if (max_time < dt[i]) { crd = (int8_t)i; max_time = dt[i]; } // 懈褖械屑 芯褋褜 褋 屑邪泻褋懈屑邪谢褜薪褘屑 胁褉械屑械薪械屑 褍褋泻芯褉械薪懈褟      
	}
	if (crd >= 0 && df[crd] < _smParam.smoothStopF_to0[crd] && step_df[crd] == _smParam.smoothAF[crd]) return -1; // 械褋谢懈 褉邪蟹薪懈褑邪 胁 褋泻芯褉芯褋褌褟褏 屑械薪褜褕械 褌芯泄, 锌褉懈 泻芯褌芯褉芯泄 屑芯卸薪芯 褋褌邪褉芯褌芯胁邪褌褜 褋 0
	*smothdF = frq_out[crd] < (int32_t)p->frq[crd] ? step_df[crd] : -step_df[crd];
	*frqStop = frq_out[crd];
	return crd;
}

// 懈薪褌械谐褉懈褉芯胁薪懈械 褔懈褋谢芯胁芯械, 褔褌芯 斜褘 锌褉芯胁械褉懈褌褜 械褋谢懈 谢懈 胁芯蟹屑芯卸薪芯褋褌褜 蟹邪褌芯褉屑芯蟹懈褌褜 薪邪 泻芯褉芯褌泻懈褏 芯褌褉械蟹泻邪褏
